...
function renderCards(resources) {
    ...
    resources.forEach((resource, index) => {
        ...
        if (resource.category === 'essential' && resource.subcategory) {
            ...
            subcategoryLabel = `<span class="absolute top-2 right-2 bg-white text-primary text-xs px-2 py-1 rounded-full z-10 shadow-md">${subcategoryText}</span>`;
        }
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1600 900">
          <defs>
            <linearGradient id="blueGradient_${index}" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" style="stop-color:${getRandomBlueShade(0)};stop-opacity:1" />
              <stop offset="100%" style="stop-color:${getRandomBlueShade(1)};stop-opacity:1" />
            </linearGradient>
          </defs>
          <rect fill="url(#blueGradient_${index})" width="1600" height="900"/>
          <!-- 添加简单几何图形作为背景装饰 -->
          <circle cx="300" cy="200" r="80" fill="rgba(255,255,255,0.1)" />
          <circle cx="1300" cy="700" r="120" fill="rgba(255,255,255,0.1)" />
          <rect x="1200" y="150" width="100" height="100" rx="20" fill="rgba(255,255,255,0.1)" />
          <rect x="200" y="650" width="150" height="150" rx="30" fill="rgba(255,255,255,0.1)" />
          <!-- 标题文本 -->
          <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="SF Pro Display, sans-serif" font-size="110" fill="#ffffff">${title.length > 10 ? title.substring(0, 10) + '...' : title}</text>
        </svg>
        `;
        
        // 将SVG转换为data URL
        const svgDataUrl = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(customPlaceholder);
        
        card.innerHTML = `
            <a href="${resource.url}" target="_blank" rel="noopener noreferrer" class="relative">
                ${subcategoryLabel}
                <div class="card-img-container">
                    <img src="${svgDataUrl}" alt="${title}" loading="lazy">
                </div>
...
// 获取随机蓝色阴影
function getRandomBlueShade(position) {
    // 蓝色色彩库，分为两组：深色（起始位置）和浅色（结束位置）
    const darkBlues = ['#1a65c9', '#0047AB', '#0F52BA', '#2B60DE', '#1F45FC', '#1E56B0'];
    const lightBlues = ['#0096ff', '#39B5FF', '#0CC0DF', '#50C4F5', '#38C1F0', '#50BFE6'];
    
    // 根据位置返回对应色组中的随机颜色
    if (position === 0) {
        // 返回深色蓝（渐变起始颜色）
        return darkBlues[Math.floor(Math.random() * darkBlues.length)];
    } else {
        // 返回浅色蓝（渐变结束颜色）
        return lightBlues[Math.floor(Math.random() * lightBlues.length)];
    }
} 